...
/// Page de chat libre avec MACI — séparée du [AiDashboardPage].
///
...
/// délai simulé. Permet de tester l'UX complète du chat avant de
/// brancher l'endpoint réel `/tutor-chat` de FastAPI (roadmap M5,
/// issue #41).
///
...
/// et persister les échanges dans [AiSessionModel] via Firestore au
/// lieu de la liste locale [_messages].
...
class _AiChatPageState extends State<AiChatPage> {
  ...
  static const _repliesByKeyword = {
    'widget':
        "Un Widget est l'unité de base de toute interface Flutter. Il existe deux familles principales : `StatelessWidget` (immuable) et `StatefulWidget` (qui peut se reconstruire avec `setState()`). Tu veux un exemple de code ?",
    'stateful':
        "`StatefulWidget` est utilisé quand une partie de ton interface doit changer en réponse à une interaction ou un événement. Il fonctionne avec une classe `State` séparée qui contient les données mutables. Veux-tu que je te montre le cycle de vie complet ?",
    'firestore':
        "Firestore est une base de données NoSQL en temps réel. Dans MACIN, on l'utilise avec des `StreamBuilder` pour que l'interface se mette à jour automatiquement dès qu'un document change.",
    'quiz':
        "Pas de souci, regardons ça ensemble ! Sur quel exercice précisément bloques-tu ? Dis-moi le nom du cours ou colle-moi la question, et je t'aide à comprendre le raisonnement (pas juste la réponse 😉).",
    'bloqu':
        "Je comprends, c'est normal de buter sur un concept. Peux-tu me dire exactement à quelle étape tu es ? On va décomposer le problème ensemble.",
  };
  ...
  void _sendMessage([String? presetText]) {
    ...
    setState(() {
      _messages.add(AiMessageModel(role: 'user', content: text, timestamp: DateTime.now()));
      _isTyping = true;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          children: [
            const MaciAvatar(size: 32),
            const SizedBox(width: AppDimensions.sm),
            Text('MACI', style: AppTextStyles.heading2),
          ],
        ),
      ),
      body: Column(
